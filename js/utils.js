...
let running=false;
window.__ktsRunPageTransition=async function(swap,destinationId){if(running)return;const active=transitionFor(destinationId);if((active.exitType||active.type)==='none'&&(active.enterType||active.type)==='none'||reduced||!Element.prototype.animate){swap();currentPageId=destinationId||currentPageId;return}running=true;const root=config.preview?document.querySelector('.kts-preview-stage'):document.body;let swapped=false;const applySwap=()=>{if(!swapped){swapped=true;swap();currentPageId=destinationId||currentPageId}};try{await root.animate(frames(false,active),{duration:active.duration/2,easing:active.easing,fill:'both'}).finished;if(active.delay)await new Promise(resolve=>setTimeout(resolve,active.delay));applySwap();await root.animate(frames(true,active),{duration:active.duration/2,easing:active.easing,fill:'both'}).finished}catch(error){applySwap()}finally{running=false}};
