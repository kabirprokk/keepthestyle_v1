...
function generatePageTransitionRuntime(settings = {}, options = {}) {
    ...
    const config = {
        ...
        routes: (settings.pageTransitions || []).map(route => ({ fromId: route.fromId, toId: route.toId, type: allowed.includes(route.type) ? route.type : 'fade', duration: Math.min(2000, Math.max(100, Number(route.duration) || 450)), easing: easingOptions.includes(route.easing) ? route.easing : 'ease-in-out' })),
...
const frames=(entering,active)=>{const presets={fade:[{opacity:0},{opacity:1}],'slide-left':[{opacity:0,transform:'translateX(8%)'},{opacity:1,transform:'translateX(0)'}],'slide-right':[{opacity:0,transform:'translateX(-8%)'},{opacity:1,transform:'translateX(0)'}],'slide-up':[{opacity:0,transform:'translateY(8%)'},{opacity:1,transform:'translateY(0)'}],zoom:[{opacity:0,transform:'scale(.94)'},{opacity:1,transform:'scale(1)'}],blur:[{opacity:0,filter:'blur(12px)'},{opacity:1,filter:'blur(0)'}],flip:[{opacity:0,transform:'perspective(900px) rotateY(10deg)'},{opacity:1,transform:'perspective(900px) rotateY(0)'}]};const pair=presets[active.type]||presets.fade;return entering?pair:[pair[1],pair[0]]};
let running=false;
window.__ktsRunPageTransition=async function(swap,destinationId){if(running)return;const active=transitionFor(destinationId);if(active.type==='none'||reduced||!Element.prototype.animate){swap();currentPageId=destinationId||currentPageId;return}running=true;const root=config.preview?document.querySelector('.kts-preview-stage'):document.body;let swapped=false;const applySwap=()=>{if(!swapped){swapped=true;swap();currentPageId=destinationId||currentPageId}};try{await root.animate(frames(false,active),{duration:active.duration/2,easing:active.easing,fill:'both'}).finished;applySwap();await root.animate(frames(true,active),{duration:active.duration/2,easing:active.easing,fill:'both'}).finished}catch(error){applySwap()}finally{running=false}};
